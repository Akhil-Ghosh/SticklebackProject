...
In addition, we assume that the discrete traits abdominal vertebrae (mav) and caudal vertebrae (mcv) also have correlation with the standard length of the fish (\textbf{CITATION}). If $Y_{k,ij}$ is one of the above traits, then
\begin{align}
...
\label{eq:priors}
\end{align}
We also note that, for the discrete phenotypes in equations (\ref{eq:disc_corr}) and (\ref{eq:disc_ind}), there is no $\sigma_k$, and thus is not sampled for those phenotypes.
...
# References
>>>>>>> 471fb0b0ba3b93e818ccc4987a5a22f419617cec
