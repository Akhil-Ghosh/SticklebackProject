...
For our study, we inferred sex in *G. doryssus* fossils by comparing multivariate morphological trait data from lineage II samples to the same multivariate trait set collected from multiple populations in the closely-related, extant Threespine Stickleback species complex (Gasterosteus aculeatus). Crucially, we determined extant *Gasterosteus aculeatus* sex directly via dissection and/or PCR genotyping. This enabled us to use Multiple Imputation by Chained Equations (MICE; @MICE) to build a predictive multiple imputation algorithm (@little2002statistical) based on the multivariate morphology of *G. aculeatus* individuals of known sex. We applied this algorithm to the fossil data to impute individual sex based on morphology, treating fossil sex as a missing variable. With sex assigned to fossil specimens, we fit a modified Ornstein-Uhlenbeck (OU) (@OUProcess) model using a Bayesian framework that accounts for uncertainty in sex classification to estimate trait means through time. Then we quantified the evolution of sexual dimorphism in each trait over ~16,000 years of lineage II.
...
## Missing data imputation to classify fossil sex
Fossil and extant data were combined to create a single data set which contained small amounts of missingness on the observed traits \textcolor{red}{Greg, THIS MAY NOT BE TRUE. SOME FOSSIL TRAITS HAD LOTS OF MISSINGNESS. IS THIS A PROBLEM?} with the exception of sex, which was completely missing for fossil specimens as this was unobservable.  WE imputed sex and the rest of the missing data in the same step using multiple imputation (@little2002statistical), performed via the "multiple imputation by chained equations" (MICE) algorithm (@MICE) with predictive mean matching (@hotdeck), and implemented using the R programming language (@R2022language). Traditionally, the choice for the number of completed data sets is a relatively small number such as $M = 5$ or $M = 10$.  However, @ZhouReiter2010 recommend a larger number of imputed data sets if users intend on performing Bayesian analysis, as was performed here, after imputation.  Therefore, we imputed $M = 100$ complete data sets.  Extant fish records were then dropped from each of the imputed data sets, and only the fossil data were used in downstream analyses of fossil sexual dimorphism. 
To test the accuracy of our multiple imputation model in estimating classifying sex,wWe performed 5-fold cross validation using the extant cases only. For a given fold, we took the observed values of sex and removed them \textcolor{red}{Greg, HOW? I GUESS WE REMOVED SEX FOR EVERY INDIVIDUAL?} to reproduce the missingness observed in the fossil data.  Then with one fold held out, we used the remaining folds with observed sex to perform multiple imputation with predictive mean matching, as before, with $M$ = 5,  to build a model that we then used to impute the sex of each fish in the held-out fold (along with any other variables that needed to be imputed).  We then compared the imputed values of sex in each held-out fold to the true values of sex to evaluate the accuracy of the imputation model, and combined the output to estimate the probability of being correctly called male versus female (i.e. the probability of being male was estimated as the proportion of imputations that predicted male \textcolor{red}{Greg, THE PROPORTION OF IMPUTATIONS THAT PREDICTED MALE ***CORRECTLY***?}).  We constructed an ROC curve from these probabilities (Figure \ref{fig:roc}) to visualize imputation accuracy. 

\textcolor{red}{THIS STATEMENT ABOUT POOLING DRAWS SEEMS OUT OF PLACE. SHOULDN'T IT GO JUST BEFORE WE TALK ABOUT USING THE POOLED DRAWS?}
After performing the Bayesian analysis on each of the $M = 100$ imputed data sets, draws from the posterior distribution based on each of the imputed data sets were pooled together to combine posterior draws across imputations as recommended in @ZhouReiter2010 rather than using Rubin's traditional combining rules.  
...
<!-- In the above model descriptions, $\mu_{k,ft_i}$ and $\mu_{k,mt_i}$ model the time-$t_i$, sample-specific (potentially log-transformed) trait mean $k$ for females and males, respectively. -->  To account for the possibility that there is a time-dependent (i.e., evolutionary) trend in $\mu_{k,ft_i}$ and $\mu_{k,mt_i}$, we further set
\begin{align}
...
We quantify sexual dimorphism at each of the 18 temporal samples $t_i: i = 1,\cdots,T$ by calculating the difference in mean trait size for males and females at each time for a continuous phenotype, $SD_{k,t_i} = \mu_{k,m,t_i} - \mu_{k,f,t_i}$ and for a discrete phenotype $SD_{k,t_i} = \lambda_{k,m,t_i} - \lambda_{k,f,t_i}$. That is, we subtract female means from male means. For simplicity, we will define sexual dimorphism for both continuous and discrete traits as $SD_{k,t_i} = \mu_{k,m,t_i} - \mu_{k,f,t_i}$. Because we are performing a Bayesian analysis, we do not obtain a single point estimate of sexual dimorphism at a particular time, but rather $R$ samples from the posterior distribution of sexual dimorphism given the observed fossil and extant data \textcolor{red}OKAY, HERE AGAIN WE TALK ABOUT R SAMPLES DRAWN FROM THE IMPUTATIONS. THIS IS GETTING CONFUSING.}. Therefore, our analysis will be conducted by calculating $SD_{k,t_i,r} = \mu_{k,m,t_i,r} - \mu_{k,f,t_i,r}$ for $r = 1,\dots,R$ and then perfoming our data analysis on these samples. Presence (or absense) of sexual dimorphism for trait $k$ can be indicated by estimating the posterior probability of positive sexual dimorphism $P(SD_{k,t_i} > 0 | \boldsymbol{y}) = \frac{1}{R} \sum_{r=1}^R I(SD_{k,t_i,r} > 0)$. Figures \ref{fig:post_probs_armor} and \ref{fig:post_probs_nonarmor}  plot these posterior probabilities across time for armored traits and non-armored traits, respectively. The closer the $P(SD_{k,t_i} > 0 | \boldsymbol{y})$ is to 1, the larger the evidence that the average trait size for males is larger than females, while if $P(SD_{k,t_i} > 0 | \boldsymbol{y})$ is close to 0, the larger the evidence that the average trait size for females is larger than males, both indicating sexual dimorphism. On the contrary, $P(SD_{k,t_i} > 0 | \boldsymbol{y})$ around 0.5 indicates that the average trait size is roughly equivalent for both males and females, providing little evidence of dimorphism.
...
```{r roc, fig.cap = "Reciver-Operating Characteristic Curve comparing imputed sex to actual sex for the extant cases using 5-fold cross validation (AUC = 0.831)"}
...
```
<!-- So, what area under the ROC curve describes good discrimination? Unfortunately there is no “magic” number, only general guidelines. In general, we use the -->
<!-- following rule of thumb: -->
<!-- If -->
<!-- ⎧ -->
<!-- ⎪⎪⎪⎪⎪⎪⎪⎪⎨ -->
<!-- ⎪⎪⎪⎪⎪⎪⎪⎪⎩ -->
<!-- ROC = 0.5 This suggests no discrimination, so we might as well -->
<!-- flip a coin. -->
<!-- 0.5 < ROC < 0.7 We consider this poor discrimination, not much better -->
<!-- than a coin toss. -->
<!-- 0.7 ≤ ROC < 0.8 We consider this acceptable discrimination. -->
<!-- 0.8 ≤ ROC < 0.9 We consider this excellent discrimination. -->
<!-- ROC ≥ 0.9 We consider this outstanding discrimination. -->
We found that the AUC  was `r round(auc_rocr@y.values[[1]],3)`, indicating that our imputation model had excellent discrimination (@HosmerLemeshow, Page 177) in categorizing fish as male or female in extant *G. aculeatus* with known sex. 


```{r est-p, echo = FALSE, fig.cap = "Density estimate of the percentage of specimen that were imputted to be male across the M = 100 imputations"}
#Summaries
...
max <- dat %>% group_by(imp) %>% summarize(pct = 100*mean(gender == "Male")) %>% ungroup() %>% summarize(sd = max(pct))
...
```{r echo = FALSE}
#Ignoring time compute lambda 
library(tidyverse)
sum_p <- dat %>% group_by(imp) %>% summarize(pctmale = mean(gender == "Male"), n = n()) %>% mutate(W = pctmale*(1-pctmale)/n) %>% ungroup() %>% summarize(q = mean(pctmale), W = mean(W), B = var(pctmale)) %>% mutate(Tm = (1+1/100)*B,lambda = (1+1/100)*B/Tm, se= sqrt(Tm))

```

Across the 100 missing data imputations of the 814 fossil specimens, the average percentage of males in an imputation was `r paste0(round(meanpct$pct,1),"%")` with a standard deviation across imputations of `r paste0(round(sdpct$sd,1),"%")`, ranging from `r paste0(round(min$sd,1),"%")` to `r paste0(round(max$sd,1),"%")` Figure \ref{fig:est-p}.  In only 31 of the 100 imputations were there more males than females with the remaining 69 imputations having exactly the same number or more females. 

```{r male-prop, fig.cap = "Density estimates of the distribution of the proportions of male fish at each time period across 100 imputations"}
#Now do this across time. 
library(ggridges)
dat %>% group_by(imp, time) %>% summarize(pct_male = mean(gender == "Male")) %>% ggplot(aes(x = pct_male, y = time, group = time)) +  geom_density_ridges(fill = "#00AFBB", rel_min_height = 0.05, quantile_lines = TRUE, quantiles = 0.5) + theme_bw() + ylab("Time Period") + xlab("Male Proportion")

#Means of male proportion at each time.
#dat %>% group_by(imp, time) %>% summarize(mn = mean(gender == "Male")) %>% group_by(time) %>% summarize(pct_male = mean(mn)) %>% ggplot(aes(x = time, y = pct_male)) + geom_point() + geom_path() + theme_bw()
```

```{r}
pct_male_medians <- dat %>% group_by(imp, time) %>% summarize(mn = mean(gender == "Male")) %>% group_by(time) %>% summarize(pct_male = median(mn))
```

Figure \ref{fig:male-prop} shows density estimates of the distribution of the proportions of male fish at each time period across 100 imputations along with the median at each time point.  We can see that the first three time points have proportions of males centered around about 25\% followed by a shift up in proportion of males between time periods 5 and 9 with median values ranging from 44\% to 66.7\%.  Time periods 10 through 13 show a shift back towards more females with the median proportions of males ranging from 30.3\% to 41.5\%.  The last several time points, from 14 through 18, all have median male proportions just above 50\% with the exception of time period 17 with a median male proportion of 39.3\%.  

<!-- Finally, the fraction of missing information, $\lambda$, for the mean of all traits other than sex were all below 0.003. -->
...
temp <- data.frame(var = names(dat)[4:28],do.call(rbind,lapply(as.list(names(dat)[4:28]),greg))) %>% select(var, Q, se, lambda)
