...
For each of the $M = 100$ imputed datasets, we fit an OU model for different phenotypes of interest.  For the $i$-th observed fossil in year $t$ ($t = 1, \ldots, 18$ and  $i = 1\ldots,n_{t}$), let $W_{ti}$ be the imputed gender, $\boldsymbol{X}_{ti}$ be the $K_{cont} \times 1$ vector of continuous phenotypes, and $\boldsymbol{Y}_{ti}$ be the $K_{disc} \times 1$ vector of discrete phenotypes for.  In addition,
