...
ggsave("../Figures/SameKappa/theta.pdf",width=5,height=8)
...
as.data.frame(mu_diff) %>%
  ...
  ylab(expression(t)) +
  theme_bw()
ggsave("../Figures/SameKappa/mu_diff.pdf",width=5,height=8)
...
as.data.frame(mu_f) %>%
  ...
  ylab(expression(t)) +
  theme_bw()
ggsave("../Figures/SameKappa/mu_f.pdf",width=5,height=8)
...
as.data.frame(mu_m) %>%
  ...
  ylab(expression(t)) +
  theme_bw()
ggsave("../Figures/SameKappa/mu_m.pdf",width=5,height=8)
