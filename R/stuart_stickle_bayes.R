...
transformed parameters{
  ...
  mu_mdf[1:T] = theta_f[11] + u_mdf[1:T];
  mu_mdf[(T+1):(2*T)] = theta_m[11] + u_mdf[(T+1):(2*T)];
  mu_mav[1:T] = theta_f[12] + u_mav[1:T];
...
model{
  ...
  u_mdf[2:T] ~ normal(kappa[11]*u_mdf[1:(T-1)],tau[11]);
  u_mdf[(T+2):(2*T)] ~ normal(kappa[11]*u_mdf[(T+1):(2*T-1)],tau[11]);
  u_mav[2:T] ~ normal(kappa[12]*u_mav[1:(T-1)],tau[12]);
