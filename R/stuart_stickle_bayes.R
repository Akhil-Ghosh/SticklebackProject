...
transformed parameters{
  ...
  for (t in 1:T){
    mu_stl[t] = theta_f[1] + u_stl[t];
    mu_stl[T+t] = theta_m[1] + u_stl[T+t];
    mu_lps[t] = theta_f[2] + u_lps[t];
    mu_lps[T+t] = theta_m[2] + u_lps[T+t];
    mu_ect[t] = theta_f[3] + u_ect[t];
    mu_ect[T+t] = theta_m[3] + u_ect[T+t];
    mu_tpg[t] = theta_f[4] + u_tpg[t];
    mu_tpg[T+t] = theta_m[4] + u_tpg[T+t];
    mu_cle[t] = theta_f[5] + u_cle[t];
    mu_cle[T+t] = theta_m[5] + u_cle[T+t];
    mu_pmx[t] = theta_f[6] + u_pmx[t];
    mu_pmx[T+t] = theta_m[6] + u_pmx[T+t];
    mu_ds1[t] = theta_f[7] + u_ds1[t];
    mu_ds1[T+t] = theta_m[7] + u_ds1[T+t];
    mu_ds2[t] = theta_f[8] + u_ds2[t];
    mu_ds2[T+t] = theta_m[8] + u_ds2[T+t];
    mu_ds3[t] = theta_f[9] + u_ds3[t];
    mu_ds3[T+t] = theta_m[9] + u_ds3[T+t];
    mu_lpt[t] = theta_f[10] + u_lpt[t];
    mu_lpt[T+t] = theta_m[10] + u_lpt[T+t];
    mu_mds[t] = theta_f[11] + u_mds[t];
    mu_mds[T+t] = theta_m[11] + u_mds[T+t];
    mu_mdf[t] = theta_f[12] + u_mdf[t];
    mu_mdf[T+t] = theta_m[12] + u_mdf[T+t];
    mu_mav[t] = theta_f[13] + u_mav[t];
    mu_mav[T+t] = theta_m[13] + u_mav[T+t];
    mu_maf[t] = theta_f[14] + u_maf[t];
    mu_maf[T+t] = theta_m[14] + u_maf[T+t];
    mu_mcf[t] = theta_f[15] + u_mcf[t];
    mu_mcf[T+t] = theta_m[15] + u_mcf[T+t];
    mu_mpt[t] = theta_f[16] + u_mpt[t];
    mu_mpt[T+t] = theta_m[16] + u_mpt[T+t];
  }
}
model{
